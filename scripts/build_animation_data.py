#!/usr/bin/env python3
"""Build browser-readable LINE Rangers animation data from emulator resources.

Input layout:
  res_from_emulator/<unit_id>/<unit_id>-body.sam|plist|png
  res_from_emulator/<unit_id>/<unit_id>-bul.sam|plist|png
  res_from_emulator/<unit_id>/<unit_id>-bul2.sam|plist|png
  res_from_emulator/<unit_id>/<unit_id>-bul3.sam|plist|png

Output layout:
  res/animation_meta/index.json
  res/animation_meta/<unit_id>.json
"""
...
RESOURCE_ROOT = ROOT / "res_from_emulator"
OUT_ROOT = ROOT / "res" / "animation_meta"
...
def build_part(unit_dir: Path, prefix: str, part: str) -> dict[str, Any] | None:
    ...
    return {
        "sam": str(sam_path.relative_to(ROOT)).replace("\\", "/"),
        "plist": str(plist_path.relative_to(ROOT)).replace("\\", "/"),
    ...
    for unit_dir in sorted(path for path in RESOURCE_ROOT.iterdir() if path.is_dir()):
        try:
            ...
            units[unit_dir.name] = {
                ...
                "meta": str(out_path.relative_to(ROOT)).replace("\\", "/"),
