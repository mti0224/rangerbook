(() => {
  ...
  function levelSelect(id, section, level) {
    return `<label class="gear-level-control">
      <span class="sr-only">${escapeHtml(section)}強化等級</span>
      <select class="gear-level-select" data-gear-id="${escapeHtml(id)}" data-gear-level-section="${escapeHtml(section)}" aria-label="${escapeHtml(section)}強化等級">
        ${LEVELS.map((value) => `<option value="${value}"${value === level ? " selected" : ""}>${value === 5 ? "+Max" : `+${value}`}</option>`).join("")}
      </select>
    </label>`;
  }
  ...
  function effectTable(rows) {
    ...
    return `<div class="table-scroll gear-effect-table-wrap">
      <table class="gear-effect-table">
        <thead><tr><th>效果</th><th>數值</th></tr></thead>
        <tbody>${rows.map((row) => `<tr><th>${escapeHtml(row.effect)}</th><td>${escapeHtml(row.value)}</td></tr>`).join("")}</tbody>
      </table>
    </div>`;
  }

  function sectionTitle(section) {
    return section.querySelector(":scope > h3 > span")?.textContent.trim()
      || section.querySelector(":scope > h3")?.childNodes?.[0]?.textContent?.trim()
      || "";
  }
  ...
  function renderAdvanced(content, gear, id) {
    ...
    section.innerHTML = `${heading(id, "高級效果", level)}
      <div class="ranger-talent-list gear-advanced-detail">
        <article class="ranger-talent-card gear-advanced-card">
          ...
          ${effectTable(advancedRows(advanced, level))}
        </article>
      </div>`;
  }
  ...
  function renderSpec(content, gear, id) {
    ...
    section.innerHTML = `${heading(id, "Spec+", level)}
          ...
          <div class="talent-section gear-specplus-section gear-specplus-special-section">
            ...
            <div class="table-scroll talent-main-table-wrap">
              <table class="talent-main-table gear-specplus-condition-table">
                ...
                <tbody>${conditionValues.map((condition, index) => `<tr>${index === 0 ? `<td rowspan="${conditionValues.length}" class="talent-prob-cell">${escapeHtml(probability)}</td>` : ""}<td>${escapeHtml(condition)}</td></tr>`).join("")}</tbody>
              </table>
            </div>
            ...
            <div class="table-scroll talent-main-effect-wrap">
              <table class="talent-main-effect-table gear-specplus-effect-table">
                ...
                <tbody>${triggers.map((row) => `<tr><td>${escapeHtml(row.probability)}</td><td>${escapeHtml(row.effect)}</td><td>${escapeHtml(row.factor)}</td><td>${escapeHtml(row.time)}</td></tr>`).join("")}</tbody>
              </table>
            </div>
          </div>
        </article>
