(() => {
  const DATA_URL = "../res/labyrinthEnemy_data.json";
  ...
  async function applyTags() {
    ...
    document.querySelectorAll("#enemyList .enemy-card").forEach((card) => {
      if (card.dataset.labyrinthTagDone === "1") return;
      ...
      card.dataset.labyrinthTagDone = "1";
    });

    const modal = document.getElementById("enemyModal");
    const content = document.getElementById("enemyModalContent");
    if (!modal || modal.hidden || !content || content.dataset.labyrinthTagDone === "1") return;
    ...
    if (enemy && tags) {
      tags.appendChild(makeTag(enemy));
      content.dataset.labyrinthTagDone = "1";
    }
  }
