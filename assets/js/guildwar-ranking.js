(() => {
  ...
  const TEAMS_URL = "https://pvp-data.warmycat.com/guildwar_player_teams.json";
  ...
  function render() {
    ...
    if (!els.body) return;
    els.body.innerHTML = rows.length ? rows.map(g => `<tr class="guildwar-rank-row" data-guild-rank="${esc(g.rank)}" tabindex="0" role="button" aria-label="查看 ${esc(g.guildName || "-")} 的公會成員"><td class="pvp-rank-cell"><span class="pvp-rank-medal">${esc(g.rank)}</span></td><td><strong title="${esc(g.guildName || "-")}">${esc(g.guildName || "-")}</strong></td><td>${esc(num(g.score))}</td><td>${esc(num(g.curMemberCount))} / ${esc(num(g.maxMemberCount))}</td><td>${esc(g.nationalFlag || "-")}</td></tr>`).join("") : `<tr class="pvp-empty-row"><td colspan="5">目前沒有${esc(tier.label)}段位的公會資料。</td></tr>`;
  }

  function renderGuildMembers(guild) {
    const detail = membersByRank.get(Number(guild.rank));
    const members = Array.isArray(detail?.members) ? detail.members : [];
    const list = members.length ? `<div class="guildwar-member-list">${members.map((member, index) => `<div class="guildwar-member-row"><div><div class="guildwar-member-name">${esc(memberName(member))}</div><div class="guildwar-member-level">Lv. ${esc(memberLevel(member))}</div></div>${ADMIN_MODE ? `<button class="guildwar-member-team-button" type="button" data-member-team-index="${index}">查看進攻隊伍</button>` : ""}</div>`).join("")}</div>` : `<div class="guildwar-member-empty">此公會的成員資料尚未產生；完整成員資料每小時更新一次。</div>`;
  ...
  function normalizeTeamGroups(guildwar) {
    if (!guildwar || typeof guildwar !== "object") return [];
    return Object.entries(guildwar).filter(([, units]) => Array.isArray(units)).map(([key, units]) => [key, units.filter(unit => unit && typeof unit === "object" && unit.unitCode)]);
  }

  function renderMemberTeam(memberIndex) {
    const guild = guilds.find(g => Number(g.rank) === openGuildRank);
    const teamGuild = teamsByRank.get(openGuildRank);
    const member = Array.isArray(teamGuild?.members) ? teamGuild.members[memberIndex] : null;
    if (!guild || !member) return;
    const groups = normalizeTeamGroups(member.guildwar);
    const content = groups.length ? groups.map(([teamKey, units]) => `<section class="guildwar-team-section"><h3>${esc(teamKey)}</h3><div class="guildwar-team-grid">${units.map(unit => { const code = String(unit.unitCode || ""); const name = rangerNames[code] || code; return `<div class="guildwar-team-unit"><img src="${RANGER_IMAGE(code)}" alt="" loading="lazy" onerror="this.remove();"><strong title="${esc(name)}">${esc(name)}</strong></div>`; }).join("")}</div></section>`).join("") : `<div class="guildwar-member-empty">此成員目前沒有可顯示的公會戰進攻隊伍資料。</div>`;
    els.modalContent.innerHTML = `<button class="guildwar-team-back" type="button" data-team-back>← 返回成員列表</button><header class="guildwar-member-header"><h2 id="guildMemberModalTitle">${esc(memberName(member))}</h2><p>${esc(guild.guildName || "-")} · Lv. ${esc(memberLevel(member))}</p></header>${content}`;
  }

  function openGuild(guild) { if (!guild || !els.modal || !els.modalContent) return; openGuildRank = Number(guild.rank) || 0; renderGuildMembers(guild); els.modal.hidden = false; document.body.classList.add("modal-open"); }
  ...
  async function optionalJson(url) { try { const res = await fetch(`${url}?t=${Date.now()}`, { cache: "no-store" }); return res.ok ? await res.json() : {}; } catch { return {}; } }

  async function load() {
    status("公會排名資料載入中…");
    try {
      const requests = [fetch(`${DATA_URL}?t=${Date.now()}`, { cache: "no-store" }), optionalJson(MEMBERS_URL)];
      if (ADMIN_MODE) requests.push(optionalJson(TEAMS_URL), optionalJson(RANGERS_URL));
      const results = await Promise.all(requests);
      const rankingRes = results[0], memberData = results[1];
      ...
      if (ADMIN_MODE) {
        const teamData = results[2] || {}, rangerData = results[3] || [];
        teamsByRank = new Map((Array.isArray(teamData.guilds) ? teamData.guilds : []).map(guild => [Number(guild.rank), guild]));
        rangerNames = {};
        (Array.isArray(rangerData) ? rangerData : []).forEach(row => { const code = String(row.ranger_id || ""); if (code) rangerNames[code] = String(row["Ranger名稱"] || code); });
      }
      if (els.updated) els.updated.textContent = date(data.metadata?.generatedAtUtc);
      status(); render();
    } catch (e) { console.error(e); status("公會排名資料尚未產生或目前無法載入。", true); if (els.body) els.body.innerHTML = ""; }
  }
  ...
  els.modalContent?.addEventListener("click", e => { const teamButton = e.target.closest("[data-member-team-index]"); if (teamButton && ADMIN_MODE) { renderMemberTeam(Number(teamButton.dataset.memberTeamIndex)); return; } if (e.target.closest("[data-team-back]")) { const guild = guilds.find(g => Number(g.rank) === openGuildRank); if (guild) renderGuildMembers(guild); } });
