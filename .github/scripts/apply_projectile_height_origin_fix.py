...
old = '''  function animationDerivedMuzzle(part, readyNames, triggerNames) {
...
'''
new = '''  function isInjectedProjectileMarker(item) {
...
'''
assert text.count(old) == 1, "animationDerivedMuzzle block changed"
text = text.replace(old, new)

old = '''  function nativeStartPoint(meta, clip, projectileConfig, bodyPart) {
...
'''
new = '''  function nativeStartPoint(meta, clip, projectileConfig, bodyPart) {
...
'''
assert text.count(old) == 1, "nativeStartPoint block changed"
text = text.replace(old, new)

old = '''  function resolveReturnEnd(projectile, layout, sceneScale) {
    return {
...
'''
new = '''  function resolveReturnEnd(projectile, layout, sceneScale) {
    return {
      x: layout.bodyOriginX + layout.facing * projectile.secondStart.x * sceneScale,
      y: layout.bodyOriginY - projectile.secondStart.y * sceneScale,
    };
...
'''
assert text.count(old) == 1, "lifetime endpoint block changed"
text = text.replace(old, new)

old = '''  function resolveProjectileGeometry(projectile, bodyPart, layout, sceneScale) {
    const start = resolveStartScreen(projectile, layout, sceneScale);
    const targetHeight = selectedTargetProfile(bodyPart).contentHeight;
    const hitHeight = Number.isFinite(projectile.hitPointRate) ? targetHeight * projectile.hitPointRate : 0;
...
'''
new = '''  function resolveProjectileGeometry(projectile, bodyPart, layout, sceneScale) {
    const start = resolveStartScreen(projectile, layout, sceneScale);
    const end = resolveOutboundEnd(projectile, layout, sceneScale, start);
    const endX = end.x;
    const endY = end.y;
    const nativeDistance = Math.hypot(endX - start.x, endY - start.y) / Math.max(0.0001, sceneScale);
'''
assert text.count(old) == 1, "render endpoint block changed"
text = text.replace(old, new)

old = "一般關卡場景比例 0.85；角色與目標距離已增加 50%，縮放時目標可能移出畫面。"
new = "一般關卡場景比例 0.85；移動型投射物依原生發射高度與 endY 位移計算，目標可能位於畫面外。"
assert text.count(old) == 1, "hint text changed"
text = text.replace(old, new)
